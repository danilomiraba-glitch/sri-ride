...
def construir_totales_contexto(factura_root: etree._Element) -> dict:
    """
    Construye el bloque `totales` según la intención de `spec-totales.yml`.
    Solo aplica extracción y sumas ligeras (sin validación fiscal estricta).
    """
    impuestos = walk_path_nodes(factura_root, "factura.infoFactura.totalConImpuestos.totalImpuesto")
    ...
    propina = _read_positive_propina(factura_root)
    gastos_exportacion = _sum_paths(
        factura_root,
        [
            "factura.infoFactura.fleteInternacional",
            "factura.infoFactura.seguroInternacional",
            "factura.infoFactura.gastosAduaneros",
            "factura.infoFactura.gastosTransporteOtros",
        ],
    )
    ...
    return {
        ...
        "subtotal_general": first_text(factura_root, "factura.infoFactura.totalSinImpuestos"),
        "total_descuento": first_text(factura_root, "factura.infoFactura.totalDescuento"),
        ...
        "total": first_text(factura_root, "factura.infoFactura.importeTotal"),
    }
...
def _sum_paths(factura_root: etree._Element, dot_paths: list[str]) -> Decimal | None:
    total = Decimal("0")
    found = False
    for path in dot_paths:
        value = first_text(factura_root, path.lstrip("-").strip())
...
def _read_positive_propina(factura_root: etree._Element) -> Decimal | None:
    raw = first_text(factura_root, "factura.infoFactura.propina")
    dec = _to_decimal(raw)
