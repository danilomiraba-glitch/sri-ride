...
def normalizar_nota_credito(nota_credito_obj: Any) -> dict:
    info_tributaria = _first_node(nota_credito_obj, "infoTributaria")
    info_nota_credito = _first_node(nota_credito_obj, "infoNotaCredito")
    detalles = _walk_path_nodes(nota_credito_obj, "detalles.detalle")
    campos_adicionales = _walk_path_nodes(nota_credito_obj, "infoAdicional.campoAdicional")
    impuestos = _walk_path_nodes(nota_credito_obj, "infoNotaCredito.totalConImpuestos.totalImpuesto")
    string_fields = _walk_path_nodes(nota_credito_obj, "extensions.extension.StringField")

    return {
        "doc": _build_doc_context(info_tributaria, info_nota_credito, string_fields),
        "emisor": _build_emisor_context(info_tributaria, info_nota_credito),
        "cliente": _build_cliente_context(info_nota_credito),
        "producto": _build_producto_context(detalles),
        "info_adicional": _build_info_adicional_context(campos_adicionales),
        "totales": _build_totales_context(info_nota_credito, impuestos),
        "comprobante": _build_comprobante_context(info_nota_credito),
    }


def _build_doc_context(info_tributaria: Any, info_nota_credito: Any, string_fields: list[Any]) -> dict:
    numero = _join_values(
        [
    ...
    return {
        ...
        "cntrbynte_esp": _first_text_from(info_nota_credito, "contribuyenteEspecial"),
        "agent_retencn": _first_text_from(info_tributaria, "agenteRetencion"),
    }


def _build_emisor_context(info_tributaria: Any, info_nota_credito: Any) -> dict:
    regimen = _first_text_from(info_tributaria, "contribuyenteRimpe")
    if not regimen:
        regimen = "General"

    obligado_raw = _first_text_from(info_nota_credito, "obligadoContabilidad")
    obligado = (obligado_raw or "").upper() if obligado_raw else obligado_raw

    return {
        "direccion": _first_text_from(info_tributaria, "dirMatriz"),
        "direccion_estab": _first_text_from(info_nota_credito, "dirEstablecimiento"),
...
def _build_cliente_context(info_nota_credito: Any) -> dict:
    return {
        "razon_social": _first_text_from(info_nota_credito, "razonSocialComprador"),
        "identificacion": _first_text_from(info_nota_credito, "identificacionComprador"),
        "fecha_emision": _first_text_from(info_nota_credito, "fechaEmision"),
    }
...
def _build_totales_context(info_nota_credito: Any, impuestos: list[Any]) -> dict:
    ...
    return {
        ...
        "subtotal_general": _first_text_from(info_nota_credito, "totalSinImpuestos"),
        "total_descuento": _first_text_from(info_nota_credito, "totalDescuento"),
        ...
        "total": _first_text_from(info_nota_credito, "valorModificacion"),
    }


def _build_comprobante_context(info_nota_credito: Any) -> dict:
    tipo_comprobante_codigo = _first_text_from(info_nota_credito, "codDocModificado")
    ...
    return {
        ...
        "numero_comprobante": _first_text_from(info_nota_credito, "numDocModificado"),
        "fecha_comprobante": _first_text_from(info_nota_credito, "fechaEmisionDocSustento"),
        "motivo": _first_text_from(info_nota_credito, "motivo"),
    }
